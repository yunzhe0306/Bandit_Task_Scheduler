...
class ANIL(nn.Module):
    ...
    def forward_val(self, xs, ys, xq, yq, fast_weights):
        ...
        fast_weights_logits = OrderedDict(
            {"weight": fast_weights['logits.weight'], "bias": fast_weights['logits.bias']})

        for inner_batch in range(self.args.num_updates):
            logits = self.learner.functional_forward_val(xs, fast_weights, fast_weights_logits,
        ...
        query_logits = self.learner.functional_forward_val(xq, fast_weights, fast_weights_logits,
                                                           is_training=True).squeeze()
        query_loss = self.loss_fn(query_logits, yq)

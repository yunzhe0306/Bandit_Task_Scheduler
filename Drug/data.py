...
class OurMetaDataset(Dataset):
    def __init__(self, args, target_assay):
        ...
        """
        self.data_path = os.path.join(args.data_dir, args.dataset_path)
        self.compound_filename = os.path.join(args.data_dir, args.compound_filename)
        self.type_filename = os.path.join(args.data_dir, args.type_filename)
        self.fp_filename = os.path.join(args.data_dir, args.fp_filename)
    ...
    def load_dataset(self):
        ...
        self.split_name_train_val_test = pickle.load(
            open(os.path.join(self.args.data_dir, 'drug_split_id_group{}.pickle'.format(self.args.drug_group)), 'rb'))
        ...
        self.data_length['train_val'] = 4100
        self.train_indices = []
        self.val_indices = []
        self.test_indices = []
        ...
        for idx, assay_id in enumerate(shuffled_assay_ids):
        # for idx, assay_id in tqdm.tqdm(enumerate(shuffled_assay_ids)):
            if idx % 500 == 0: print("idx = ", idx)
            sys.stdout.flush()
            X_supp_tgt = {}
            y_supp_tgt = {}
            assay_supp_tgt = {}
    ...
    def get_set(self, dataset_name, seed, idx):
        ...
        if dataset_name == 'train_val':
            if self.args.sampling_method == 'ATS':
                task_num = self.args.buffer_size
            elif self.args.sampling_method == 'Bandit':
                task_num = self.args.buffer_size
            else:
                task_num = self.batch_size
            selected_indices = rng.choice(self.train_indices,
                                          size=task_num, replace=False)
            rng.shuffle(selected_indices)

            for si in selected_indices:
                support_set_x.append(self.X_train[si]['support'])
                support_set_y.append(self.y_train[si]['support'] + self.noise * \
                                     np.random.normal(size=self.y_train[si]['support'].shape))
            ...
            support_set_y_val = []
            support_set_z_val = []
            support_set_assay_val = []
            target_set_x_val = []
            target_set_y_val = []
            target_set_z_val = []
            target_set_assay_val = []
            ...
            for si in selected_indices:
                support_set_x_val.append(self.X_val[si]['support'])
                support_set_y_val.append(self.y_val[si]['support'])
                support_set_z_val.append(self.type_val[si]['support'])
                support_set_assay_val.append(self.assay_val[si])

                target_set_x_val.append(self.X_val[si]['target'])
                target_set_y_val.append(self.y_val[si]['target'])
                target_set_z_val.append(self.type_val[si]['target'])
                target_set_assay_val.append(self.assay_val[si])
            ...
            return support_set_x, support_set_y, support_set_z, support_set_assay, \
                   ...
                   target_set_x_val, target_set_y_val, target_set_z_val, target_set_assay_val, seed
    ...
    def __getitem__(self, idx):
        if self.current_set_name == 'train_val':
            support_set_x, support_set_y, support_set_z, support_set_assay, \
            target_set_x, target_set_y, target_set_z, target_set_assay, seed, \
            support_set_x_val, support_set_y_val, support_set_z_val, support_set_assay_val, \
            target_set_x_val, target_set_y_val, target_set_z_val, target_set_assay_val, seed_val = \
                self.get_set('train_val', seed=self.seed['val'] + idx, idx=idx)
            return support_set_x, support_set_y, support_set_z, support_set_assay, \
               target_set_x, target_set_y, target_set_z, target_set_assay, seed, \
               support_set_x_val, support_set_y_val, support_set_z_val, support_set_assay_val, \
               target_set_x_val, target_set_y_val, target_set_z_val, target_set_assay_val, seed_val
...
class MetaLearningSystemDataLoader(object):
    ...
    def get_dataloader(self):
        ...
        :return:
        """
        return DataLoader(self.dataset, batch_size=1, num_workers=0, shuffle=False, drop_last=True)
    ...
    def get_train_val_batches(self, total_batches=-1):
        """
        Returns a training batches and validation batches data_loader, specifically for mentornet
        ...
        self.dataset.switch_set(set_name="train_val", current_iter=self.total_train_iters_produced)

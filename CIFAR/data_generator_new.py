...
class CIFAR_100(Dataset):
    ...
    def __init__(self, args, mode, tau=0.7, alpha=3):
        ...
        #
        save_dir = '../data/cifar100/processed'

        # data-split: 64-16-20
        if mode == 'train':
            self.data_file = '{}/train_data.npy'.format(save_dir)
            self.data_file_val = '{}/val_data.npy'.format(save_dir)
            if self.noise > 0:
            ...
            self.data_file = '{}/val_data.npy'.format(save_dir)
            self.noise = 0
        elif mode == 'test':
            self.data_file = '{}/test_data.npy'.format(save_dir)
            self.noise = 0

        self.data = np.load(self.data_file)
        self.data = torch.tensor(np.transpose(self.data, (0, 1, 4, 2, 3))).float()
        ...
        if mode == 'train':
            if args.limit_data:
                ...
                self.noisy_tasks = self.random_state.choice(chosen_classes_idx,
                ...
                if self.copy_noisy_task_flag:
                    ...
                    self.noisy_tasks = np.load('./noisy_task_indices/noisy_tasks_FOR_ALL_{}.npy'.format(str(args.noisy_task_ratio)))
                    print(" ---------------- Noisy tasks: ", self.noisy_tasks)
        else:
            self.noise = 0
        ...
        if self.skewed_task_distribution_flag:
            skew_pattern = args.skew_pattern
            if skew_pattern == 1:
                # Most skewed
                self.skewed_sampling_prob = np.concatenate([0.1 * np.ones(5),
                                                            0.05 * np.ones(5),
                                                            (0.25 / (self.data.shape[0] - 10)) * np.ones(
                                                                self.data.shape[0] - 10)])
            elif skew_pattern == 2:
                # Standard skewed
                self.skewed_sampling_prob = np.concatenate([0.08 * np.ones(5),
                                                            0.03 * np.ones(5),
                                                            (0.45 / (self.data.shape[0] - 10)) * np.ones(
                                                                self.data.shape[0] - 10)])
            elif skew_pattern == 3:
                # Less skewed
                self.skewed_sampling_prob = np.concatenate([0.05 * np.ones(5),
                                                            0.02 * np.ones(5),
                                                            (0.65 / (self.data.shape[0] - 10)) * np.ones(
                                                                self.data.shape[0] - 10)])
            else:
                self.skewed_sampling_prob = None
    ...
    def reset_random_state(seed=None):
        if seed is None:
            self.random_state = np.random.RandomState(self.args.seed)
        else:
    ...
    def __getitem__(self, index):
        ...
        support_x = torch.FloatTensor(torch.zeros((self.args.meta_batch_size, self.set_size, 3, 32, 32)))
        query_x = torch.FloatTensor(torch.zeros((self.args.meta_batch_size, self.query_size, 3, 32, 32)))
        ...
        for meta_batch_id in range(self.args.meta_batch_size):
            ...
            if not (noise > 0 and self.mode == 'train'):
                ...
                x = torch.zeros((self.set_size + self.query_size, 3, 32, 32))
...
class CIFAR_100_GCP(Dataset):

    def __init__(self, args, mode, tau=0.7, alpha=3):
        super(CIFAR_100_GCP, self).__init__()
        self.args = args
        self.nb_classes = args.num_classes
        self.nb_samples_per_class = args.update_batch_size + args.update_batch_size_eval
        self.n_way = args.num_classes  # n-way
...
class CIFAR_Noisy_Task(Dataset):
    def __init__(self, args, mode, diverse=True):
        super(CIFAR_Noisy_Task, self).__init__()
        self.args = args
        self.noise = args.noise
        self.nb_classes = args.num_classes
        self.nb_samples_per_class = args.update_batch_size + args.update_batch_size_eval
        ...
        self.data = torch.tensor(np.transpose(np.load(self.data_file), (0, 1, 4, 2, 3))).float()
        ...
        if mode == 'train':
            self.data_val = torch.tensor(np.transpose(np.load(self.data_file_val), (0, 1, 4, 2, 3))).float()
            # self.data_val = torch.tensor(np.transpose(self.data_val, (0, 1, 4, 2, 3)))
            self.classes_idx_val = np.arange(self.data_val.shape[0])

        # self.data = torch.transpose(self.data, (0, 1, 4, 2, 3))
        self.classes_idx = np.arange(self.data.shape[0])
        self.samples_idx = np.arange(self.data.shape[1])
    ...
    def get_sampled_data(self, data, index, setting):
        ...
        support_x = torch.FloatTensor(torch.zeros((task_num, self.set_size, 3, 32, 32)))
        query_x = torch.FloatTensor(torch.zeros((task_num, self.query_size, 3, 32, 32)))
...
class CIFAR_Error_Analysis(Dataset):
    def __init__(self, args, mode, noisy_tasks=None, skew_pattern=1):
        super(CIFAR_Error_Analysis, self).__init__()
        self.args = args
        self.noise = args.noise
        self.nb_classes = args.num_classes
        self.nb_samples_per_class = args.update_batch_size + args.update_batch_size_eval

...
class ANIL(nn.Module):
    def __init__(self, args, final_layer_size, stride, use_FOCAL_LOSS=False, GCP_flag=False, device=None):
        super(ANIL, self).__init__()
        self.args = args
        self.GCP_flag = GCP_flag
        ...
        if use_FOCAL_LOSS:
            self.loss_fn_val = FocalLoss(class_num=args.num_classes, device=device)
    ...
    def forward(self, xs, ys, xq, yq, refined_acc=False):
        create_graph = True
        ...
        refined_acc_val = (y_pred == yq).float()

        if self.GCP_flag:
            return query_loss, query_acc, query_logits.softmax(dim=1)
        else:
            if refined_acc:
                return query_loss, refined_acc_val
            else:

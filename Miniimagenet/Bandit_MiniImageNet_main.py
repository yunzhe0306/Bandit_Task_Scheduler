...
from data_generator import MiniImagenet, MiniImagenet_Noisy_Task, MiniImagenet_Error_Analysis

sys.path.append(os.path.abspath(".."))
from Bandit_Adaptive_Sampler_pkg.Bandit_Adaptive_Sampler_Mini_ImageNet import Bandit_Sampler
from Bandit_Adaptive_Sampler_pkg.Bandit_Config_Mini_ImageNet import get_bandit_sampler_config
...
parser.add_argument('--datasource', default='miniimagenet', type=str,
                    help='sinusoid or omniglot or mixture or metadataset or metadataset_leave_one_out or multiscale')
...
parser.add_argument('--num_filters', default=32, type=int,
                    help='number of filters for conv nets -- 32 , 64 for omiglot.')
...
parser.add_argument('--datadir', default='../data/miniimagenet/miniimagenet', type=str, help='directory for datasets.')
...
now = datetime.now()
dt_string = now.strftime("%d-%m-%Y_%H-%M-%S") + '_{}_{}_{}_{}_{}'.format(str(args.noise), str(args.noisy_task_ratio),
                                                                         str(args.copy_noisy_task_flag),
                                                                         str(args.num_classes),
                                                                         str(args.skewed_task_distribution_flag))
...
def train(args, anil, optimiser, bandit_config_parser, run_i):
    ...
    if args.datasource.lower() == 'miniimagenet':
        if args.finetune:
            ...
            if args.sampling_method == 'Bandit':
                dataloader = MiniImagenet_Noisy_Task(args, mode='train')
            else:
                dataloader = MiniImagenet(args, 'train')

    # Error analysis data loader
    train_full_data_loader = MiniImagenet_Error_Analysis(args=args, mode='train', noisy_tasks=dataloader.noisy_tasks)
    test_full_data_loader = MiniImagenet_Error_Analysis(args=args, mode='test')
    ...
    for step, data in enumerate(dataloader):
        ...
        if step != 0 and step % Save_Iter == 0:
            ...
            # _, __ = test(args, en_model, step, ensemble_test_flag=False)
...
def test(args, anil, test_epoch, ensemble_test_flag=False):
    ...
    dataloader = MiniImagenet_Noisy_Task(args, mode='test')
...
def main():
    ...
    if args.train == 1:
        with torch.backends.cudnn.flags(enabled=False):
            run_encoding = "00"
            final_acc = train(args, anil, meta_optimiser, bandit_config_parser=bandit_config_parser,
                              run_i=run_encoding)

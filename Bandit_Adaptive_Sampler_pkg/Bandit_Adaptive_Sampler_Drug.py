...
class FC_Net(nn.Module):
    def __init__(self, input_dim, hidden_size, buffer_size=20000, lr_rate=0.0001, device=None):
...
class Bandit_Sampler(nn.Module):
    def __init__(self, input_dim, hidden_size, alpha=0.5, g_pool_step_meta=-1, g_pool_step_bandit=-1, buffer_size=20000,
                 lr_rate=0.0001, artificial_exp_score=-1, device=None):
        ...
        #
        self.batch_loss_func = nn.MSELoss(reduce=False, reduction='none')
    ...
    def calculate_meta_loss_with_tasks(self, tasks, meta_model):
        ...
        for this_task in tasks:
            ...
            x1, y1, x2, y2 = x1.squeeze(0).float().to(self.device), y1.squeeze(0).float().to(self.device), \
                             x2.squeeze(0).float().to(self.device), y2.squeeze(0).float().to(self.device)
            loss_val = meta_model(x1, y1, x2, y2)
    ...
    def get_meta_params_with_tasks(self, meta_model, task_indices, tasks, indices_layer_wird_grad=None):
        ...
        for this_task in tasks:
            ...
            support_x, support_y, query_x, query_y = \
                support_x.squeeze(0).float().to(self.device), support_y.squeeze(0).float().to(self.device), \
                query_x.squeeze(0).float().to(self.device), query_y.squeeze(0).float().to(self.device)
            support_count, query_count = support_x.shape[0], query_x.shape[0]

            # Get inner loop weights -----------------------
            loss_support = self.batch_loss_func(meta_model.learner(support_x).reshape(-1), support_y).mean()
    ...
    def cal_true_reward_sampling(self, meta_model, tasks, selected_indices, new_tasks_num, selected_overall_score,
        ...
        for task_i, this_task in enumerate(tasks):
            ...
            for val_task_i in val_taks_indices:
                ...
                ####
                inner_logits = \
                    meta_model.learner.functional_forward_val(support_x, initial_loop_weights, initial_loop_weights_logits,
                                                              is_training=True).squeeze()
    ...
    def scale_transform(self, source):
        ...
        return 10 * torch.exp(-0.1 * torch.exp(1 * source))

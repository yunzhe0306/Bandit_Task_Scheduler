...
class FC_Net(nn.Module):
    def __init__(self, input_dim, hidden_size, buffer_size=20000, lr_rate=0.0001, embedding_layer_hidden_size=-1,
                 device=None):
        ...
        if embedding_layer_hidden_size <= 0:
            ...
            layer_list = [nn.Linear(input_dim, embedding_layer_hidden_size), nn.ReLU(),
                          nn.Linear(embedding_layer_hidden_size, hidden_size)]
...
class Bandit_Sampler(nn.Module):
    def __init__(self, input_dim, hidden_size, alpha=0.5, g_pool_step_meta=-1, g_pool_step_bandit=-1, buffer_size=20000,
        ...
        if embedding_layer_hidden_size <= 0:
            if g_pool_step_meta > 0:
                meta_param_num = int((input_dim // g_pool_step_meta))
            else:
            ...
            meta_param_num = input_dim
            self.exploit_net_f_1 = FC_Net(meta_param_num, hidden_size, buffer_size=buffer_size, lr_rate=lr_rate,
                                          embedding_layer_hidden_size=embedding_layer_hidden_size, device=device).to(
                device)
        self.explore_net_f_2 = FC_Net(input_dim=2 * self.get_f_2_input_size(g_pool_step_bandit=g_pool_step_bandit),
                                      ...
                                      lr_rate=0.001, device=device).to(device)
        #
    ...
    def get_meta_params_with_tasks(self, meta_model, task_indices, tasks, indices_layer_wird_grad=None):
        ...
        for this_task in tasks:
            ...
            if self.g_pool_step_meta > 0:
                inner_loop_weight_vec = F.avg_pool1d(inner_loop_weight_vec, kernel_size=2 * self.g_pool_step_meta,
                                                     stride=self.g_pool_step_meta, ceil_mode=True).squeeze(
                    0).squeeze(0)
                outer_loop_weight_vec = F.avg_pool1d(outer_loop_weight_vec, kernel_size=2 * self.g_pool_step_meta,
                                                     stride=self.g_pool_step_meta, ceil_mode=True).squeeze(
                    0).squeeze(0)
        ...
        # Stack the contexts
        param_all_vec = nn.functional.normalize(torch.stack(param_all), dim=1)
        param_support_vec = nn.functional.normalize(torch.stack(param_support), dim=1)
        param_query_vec = nn.functional.normalize(torch.stack(param_query), dim=1)
    ...
    def sample_arms_given_probs(self, probs, batch_num, duplicate=False):
